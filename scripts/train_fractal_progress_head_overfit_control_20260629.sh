...
case "$VARIANT" in
  stronger_bs64_5k)
    EXPERIMENT_SUFFIX="scalar03_bs64_5k_current_lr5e6_wd002_noreg_${STAMP}"
    export GLOBAL_BATCH_SIZE="${GLOBAL_BATCH_SIZE:-64}"
    export GRADIENT_ACCUMULATION_STEPS="${GRADIENT_ACCUMULATION_STEPS:-1}"
    ;;
  stronger_bs256_accum_5k)
    EXPERIMENT_SUFFIX="scalar03_bs256_accum4_5k_current_lr5e6_wd002_noreg_${STAMP}"
...
export NUM_EVAL_SHARDS_PER_EPOCH="${NUM_EVAL_SHARDS_PER_EPOCH:-8}"
...
export PROGRESS_PAIR_MARGIN_ALPHA="${PROGRESS_PAIR_MARGIN_ALPHA:-0.0}"
export PROGRESS_PAIR_SCALAR_LOSS_WEIGHT="${PROGRESS_PAIR_SCALAR_LOSS_WEIGHT:-0.3}"
export PROGRESS_LOGIT_L2_WEIGHT="${PROGRESS_LOGIT_L2_WEIGHT:-0.0}"
export PROGRESS_LOGIT_VARIANCE_WEIGHT="${PROGRESS_LOGIT_VARIANCE_WEIGHT:-0.0}"
export PROGRESS_PAIR_SMOOTHNESS_WEIGHT="${PROGRESS_PAIR_SMOOTHNESS_WEIGHT:-0.0}"
export PROGRESS_PAIR_SMOOTHNESS_MARGIN="${PROGRESS_PAIR_SMOOTHNESS_MARGIN:-0.05}"
export PROGRESS_PAIR_MONOTONIC_WEIGHT="${PROGRESS_PAIR_MONOTONIC_WEIGHT:-0.0}"
