...
OUTPUT_DIR="${OUTPUT_DIR:-output/fractal_progress_head_vlm_pooled_mlp_1k_current_tail_shrink}"
PROGRESS_TARGET="${PROGRESS_TARGET:-current}"
PROGRESS_OUTPUT_TYPE="${PROGRESS_OUTPUT_TYPE:-hard_bins}"
PROGRESS_NUM_BINS="${PROGRESS_NUM_BINS:-10}"
LOAD_BF16="${LOAD_BF16:-1}"
RESUME="${RESUME:-0}"
WANDB_PROJECT="${WANDB_PROJECT:-gr00t-progress}"
EXPERIMENT_NAME="${EXPERIMENT_NAME:-fractal_progress_head_vlm_pooled_mlp_1k_current_tail_shrink}"
...
uv run torchrun --nproc_per_node="$NUM_GPUS" --master_port="$MASTER_PORT" \
  ...
  --tune-progress-head \
  --progress-head-source vlm_pooled \
  --no-tune-projector \
